...
func (e *Election) Campaign(ctx context.Context, val string) error {
	...
	// 还是使用create_revision，实现election
	...
	// 出现leaseID相同的情况，即使用同一session，多次调用Campaign方法
	if !resp.Succeeded {
...
func (e *Election) Proclaim(ctx context.Context, val string) error {
	...
	client := e.session.Client()
	// 比对create_revision与原来的是否一致，一致即替换原来的val，否则报错ErrElectionNotLeader
...
func (e *Election) observe(ctx context.Context, ch chan<- v3.GetResponse) {
	...
	defer close(ch)
	for {
		// 根据前缀获取，按照create_revision从小到大排序
		resp, err := client.Get(ctx, e.keyPrefix, v3.WithFirstCreate()...)
		if err != nil {
			return
		}
		...
		// 如果不存在，说明在进入observe之前，主已经Resign，等待直到watch到新的主出现
		if len(resp.Kvs) == 0 {
		...
		cctx, cancel := context.WithCancel(ctx)
		// 从主的mod_revision的下一个更新版本开始watch
