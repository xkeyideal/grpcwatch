...
func init() {
	...
	// 向grpc注册resolver，scheme为endpoint
...
func (e *ResolverGroup) addResolver(r *Resolver) {
	...
	e.mu.Unlock()
	r.cc.NewAddress(addrs) // 此方法会通过接口的形式通知balancer.HandleResolvedAddrs
}
...
func ParseEndpoint(endpoint string) (proto string, host string, scheme string) {
	//fmt.Println("ParseEndpoint", endpoint)
	proto = "tcp"
	host = endpoint
